...
/**
 * Output-Filter acronym
 */
function xoutputfilter_acronym($params)
{
global $REX;
  ...
  // Ersetzen
  $x = new xoutputfilter();
  if (!isset($REX['xoutputfilter']['replaces']['3'][$REX['CUR_CLANG']]))	
    return $params['subject'];

  $search = $replace = array();

  foreach ($REX['xoutputfilter']['replaces']['3'][$REX['CUR_CLANG']] as $key => $val)
  {
    if ($REX['xoutputfilter']['excludeids']['3'][$REX['CUR_CLANG']][$key] <> '')
    {
      $exc = explode(',', $REX['xoutputfilter']['excludeids']['3'][$REX['CUR_CLANG']][$key]);
    ...
    if (trim($key)<>'' and trim($val)<>'')
    {
      ...
      foreach ($markers as $key2)
      {
        ...
        $search[] = "|(?!<[^<>]*?)(?<![?.&])\b" . $pattern . "\b(?![^<>]*?>)|msU";
        $replace[] = '<acronym title="'.$val.'">'.$key2.'</acronym>';	
      }
    }
  }
  ...
  if ($REX['ADDON']['xoutputfilter']['runtimeinfo'] == '1')
  {
    ...
    $laufzeitinfo = '<!-- XOutputFilter '.$REX['ADDON']['version']['xoutputfilter'].' [acronym] - '.$timer.' seconds -->';
  }
