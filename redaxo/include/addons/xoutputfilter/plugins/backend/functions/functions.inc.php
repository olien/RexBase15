...
/**
 * backend Plugin für XOutputFilter
 *
 ...
 */
function xoutputfilter_backend($params)
{
global $REX;
  ...
  if ($REX['ADDON']['xoutputfilter']['active'] <> '1')
    return $params['subject'];
  if (xoutputfilter_exclude_page_subpage()) // Exlude Page / Subpage prüfen
    return $params['subject'];
  ...
  $query = "SELECT * FROM $table WHERE (typ = 5) AND active = '1' ORDER BY name ASC ";
  $sql = new rex_sql;
  $sql->debugsql = 0;
  $sql->setQuery($query);
  if ($sql->getRows() == 0)
  ...
  {
    if (($sql->getValue('allcats')==1) or xoutputfilter_check_module($sql->getValue('categories')))
    {
      ...
      if (!xoutputfilter_exclude_module($exmodules))
      {
  ...
  if ($REX['ADDON']['xoutputfilter']['runtimeinfo'] == '1')
  {
    ...
    $laufzeitinfo = '<!-- XOutputFilter '.$REX['ADDON']['version']['xoutputfilter'].' [backend] - '.$timer.' seconds -->';
  }
